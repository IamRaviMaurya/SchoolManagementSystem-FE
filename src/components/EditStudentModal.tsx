"use client";

import { useState, useEffect } from "react";
import api from "@/lib/api";
import { Student } from "@/types/student";
import { 
  X, 
  Save, 
  Upload, 
  AlertTriangle, 
  CheckCircle2, 
  User,
  PhoneCall,
  GraduationCap,
  FileCheck2,
  ArrowRight,
  ArrowLeft,
  Check,
  UserCheck,
  Sparkles
} from "lucide-react";

import ImagePreviewModal from "./ImagePreviewModal";

interface EditStudentModalProps {
  student: Student;
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function EditStudentModal({
  student,
  isOpen,
  onClose,
  onSuccess,
}: EditStudentModalProps) {
  const [step, setStep] = useState<number>(1);

  const [previewModal, setPreviewModal] = useState<{
    isOpen: boolean;
    url: string | null;
    title: string;
  }>({
    isOpen: false,
    url: null,
    title: "",
  });

  const [formData, setFormData] = useState({
    first_name: student?.first_name || "",
    middle_name: student?.middle_name || "",
    last_name: student?.last_name || "",
    mother_name: student?.mother_name || "",
    dob: student?.dob || "",
    aadhar_no: student?.aadhar_no || "",
    place_of_birth: student?.place_of_birth || "",
    phone: student?.phone || "",
    email: student?.email || "",
    address: student?.address || "",
    pin_code: student?.pin_code || "",
    gender: student?.gender || "Male",
    religion: student?.religion || "Non-Minority",
    category: student?.category || "OPEN",
    photo_url: student?.photo_url || "",
    signature_url: student?.signature_url || "",
    aadhar_front_url: student?.aadhar_front_url || "",
    aadhar_back_url: student?.aadhar_back_url || "",
    division: student?.division || "School Section",
    standard: student?.standard || "1st",
    section: student?.section || "A",
    stream: student?.stream || "",
    status: student?.status || "Active",
  });

  const [proofFile, setProofFile] = useState<File | null>(null);
  const [photoPreview, setPhotoPreview] = useState<string | null>(student?.photo_url || null);
  const [signaturePreview, setSignaturePreview] = useState<string | null>(student?.signature_url || null);
  const [aadharFrontPreview, setAadharFrontPreview] = useState<string | null>(student?.aadhar_front_url || null);
  const [aadharBackPreview, setAadharBackPreview] = useState<string | null>(student?.aadhar_back_url || null);

  const [saving, setSaving] = useState<boolean>(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  // Sync state when student prop changes
  useEffect(() => {
    if (student) {
      setFormData({
        first_name: student.first_name || "",
        middle_name: student.middle_name || "",
        last_name: student.last_name || "",
        mother_name: student.mother_name || "",
        dob: student.dob || "",
        aadhar_no: student.aadhar_no || "",
        place_of_birth: student.place_of_birth || "",
        phone: student.phone || "",
        email: student.email || "",
        address: student.address || "",
        pin_code: student.pin_code || "",
        gender: student.gender || "Male",
        religion: student.religion || "Non-Minority",
        category: student.category || "OPEN",
        photo_url: student.photo_url || "",
        signature_url: student.signature_url || "",
        aadhar_front_url: student.aadhar_front_url || "",
        aadhar_back_url: student.aadhar_back_url || "",
        division: student.division || "School Section",
        standard: student.standard || "1st",
        section: student.section || "A",
        stream: student.stream || "",
        status: student.status || "Active",
      });
      setStep(1);
      setProofFile(null);
      setPhotoPreview(student.photo_url || null);
      setSignaturePreview(student.signature_url || null);
      setAadharFrontPreview(student.aadhar_front_url || null);
      setAadharBackPreview(student.aadhar_back_url || null);
      setErrorMsg(null);
    }
  }, [student]);

  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      const reader = new FileReader();
      reader.onloadend = () => {
        const base64 = reader.result as string;
        setPhotoPreview(base64);
        setFormData((prev) => ({ ...prev, photo_url: base64 }));
      };
      reader.readAsDataURL(file);
    }
  };

  const handleSignatureUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      const reader = new FileReader();
      reader.onloadend = () => {
        const base64 = reader.result as string;
        setSignaturePreview(base64);
        setFormData((prev) => ({ ...prev, signature_url: base64 }));
      };
      reader.readAsDataURL(file);
    }
  };

  const handleAadharFrontUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      const reader = new FileReader();
      reader.onloadend = () => {
        const base64 = reader.result as string;
        setAadharFrontPreview(base64);
        setFormData((prev) => ({ ...prev, aadhar_front_url: base64 }));
      };
      reader.readAsDataURL(file);
    }
  };

  const handleAadharBackUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      const reader = new FileReader();
      reader.onloadend = () => {
        const base64 = reader.result as string;
        setAadharBackPreview(base64);
        setFormData((prev) => ({ ...prev, aadhar_back_url: base64 }));
      };
      reader.readAsDataURL(file);
    }
  };

  if (!isOpen || !student) return null;

  // Check if Name or DOB was modified from original record
  const isNameChanged =
    formData.first_name !== (student.first_name || "") ||
    formData.last_name !== (student.last_name || "") ||
    formData.middle_name !== (student.middle_name || "");

  const isDobChanged = formData.dob !== (student.dob || "");

  const requiresProofDocument = isNameChanged || isDobChanged;

  const handleDivisionChange = (division: string) => {
    let defaultStd = "1st";
    let defaultStream: string | undefined = undefined;

    if (division === "Pre-Primary") {
      defaultStd = "Nursery";
    } else if (division === "School Section") {
      defaultStd = "1st";
    } else if (division === "Junior College") {
      defaultStd = "11th";
      defaultStream = "Science";
    }

    setFormData({
      ...formData,
      division,
      standard: defaultStd,
      stream: defaultStream || "",
    });
  };

  const validateCurrentStep = () => {
    setErrorMsg(null);
    if (step === 1) {
      if (!formData.first_name.trim() || !formData.last_name.trim() || !formData.mother_name.trim()) {
        setErrorMsg("Please fill in Candidate First Name, Last Name, and Mother's Name.");
        return false;
      }
    } else if (step === 2) {
      if (!formData.place_of_birth.trim() || !formData.dob.trim() || !formData.aadhar_no.trim()) {
        setErrorMsg("Please fill in Place of Birth, Date of Birth, and Aadhar Number.");
        return false;
      }
    } else if (step === 3) {
      if (!formData.address.trim() || !formData.pin_code.trim() || !formData.phone.trim()) {
        setErrorMsg("Please fill in Residential Address, Pin Code, and Primary Mobile Number.");
        return false;
      }
    }
    return true;
  };

  const handleNextStep = () => {
    if (validateCurrentStep()) {
      setStep((prev) => Math.min(5, prev + 1));
    }
  };

  const handleFinalSave = async () => {
    if (step !== 5) return;

    if (!validateCurrentStep()) return;

    if (requiresProofDocument && !proofFile) {
      setErrorMsg(
        "Mandatory Verification Required: Updating Candidate Name or Date of Birth requires uploading a valid proof document (Birth Certificate or Aadhar Card)."
      );
      return;
    }

    setSaving(true);
    try {
      await api.put(`/students/${student.id}`, formData);
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error(err);
      setErrorMsg(err.response?.data?.detail || "Failed to update student record.");
    } finally {
      setSaving(false);
    }
  };

  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] animate-fadeIn">
        {/* Modal Header */}
        <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-950">
          <div>
            <h3 className="text-lg font-bold text-white flex items-center gap-2">
              <UserCheck className="w-5 h-5 text-blue-400" />
              Edit Student Record — GR: <span className="font-mono text-blue-400">{student.gr_no}</span>
            </h3>
            <p className="text-xs text-slate-400">
              5-Step Official Candidate Update Wizard (Same layout as Admission Portal)
            </p>
          </div>
          <button
            onClick={onClose}
            className="p-2 rounded-lg bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Wizard Step Progress Bar */}
        <div className="px-6 py-4 bg-slate-950/60 border-b border-slate-800">
          <div className="flex items-center justify-between relative">
            <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-slate-800 -translate-y-1/2 z-0" />
            
            {[
              { num: 1, label: "Candidate Info", icon: User },
              { num: 2, label: "Identity & Religion", icon: UserCheck },
              { num: 3, label: "Address & Contact", icon: PhoneCall },
              { num: 4, label: "Academic Division", icon: GraduationCap },
              { num: 5, label: "Photo & Review", icon: FileCheck2 },
            ].map((item) => {
              const Icon = item.icon;
              const isDone = step > item.num;
              const isCurrent = step === item.num;
              return (
                <div
                  key={item.num}
                  onClick={() => {
                    if (item.num < step || validateCurrentStep()) {
                      setStep(item.num);
                    }
                  }}
                  className="relative z-10 flex flex-col items-center cursor-pointer"
                >
                  <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-xs transition-all ${
                      isDone
                        ? "bg-emerald-600 text-white shadow-lg shadow-emerald-500/20"
                        : isCurrent
                        ? "bg-blue-600 text-white ring-4 ring-blue-500/20 shadow-lg shadow-blue-500/30"
                        : "bg-slate-800 text-slate-400 border border-slate-700"
                    }`}
                  >
                    {isDone ? <Check className="w-4 h-4" /> : <Icon className="w-4 h-4" />}
                  </div>
                  <span
                    className={`mt-1.5 text-[10px] font-semibold ${
                      isCurrent ? "text-blue-400" : isDone ? "text-emerald-400" : "text-slate-500"
                    }`}
                  >
                    {item.label}
                  </span>
                </div>
              );
            })}
          </div>
        </div>

        {/* Form Body */}
        <form
          noValidate
          onSubmit={(e) => {
          e.preventDefault();
          handleFinalSave();
        }}
          onKeyDown={(e) => {
            if (e.key === "Enter" && (e.target as HTMLElement).tagName !== "TEXTAREA") {
              e.preventDefault();
            }
          }}
          className="p-6 space-y-6 overflow-y-auto flex-1"
        >
          {errorMsg && (
            <div className="p-4 bg-rose-500/10 border border-rose-500/30 rounded-xl flex items-center gap-3 text-rose-400 text-xs font-medium">
              <AlertTriangle className="w-5 h-5 shrink-0" />
              <span>{errorMsg}</span>
            </div>
          )}

          {/* STEP 1: Name Breakdown (4a, 4b, 4c, 4d) */}
          {step === 1 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div>
                  <h4 className="text-lg font-bold text-white">4. Candidate Full Name Breakdown</h4>
                  <p className="text-xs text-slate-400">Official name details</p>
                </div>
                <div className="text-xs font-mono font-bold text-blue-400 bg-blue-500/10 px-3 py-1 rounded-lg border border-blue-500/20">
                  GR: {student.gr_no}
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    4a. Last Name / Surname *
                  </label>
                  <input
                    type="text"
                    required
                    value={formData.last_name}
                    onChange={(e) => setFormData({ ...formData, last_name: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white uppercase font-mono text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    4b. Candidate's First Name *
                  </label>
                  <input
                    type="text"
                    required
                    value={formData.first_name}
                    onChange={(e) => setFormData({ ...formData, first_name: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white uppercase font-mono text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    4c. Middle / Father's Name
                  </label>
                  <input
                    type="text"
                    value={formData.middle_name}
                    onChange={(e) => setFormData({ ...formData, middle_name: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white uppercase font-mono text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    4d. Mother's Name *
                  </label>
                  <input
                    type="text"
                    required
                    value={formData.mother_name}
                    onChange={(e) => setFormData({ ...formData, mother_name: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white uppercase font-mono text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>
              </div>

              <div className="p-4 bg-slate-950/80 rounded-xl border border-slate-800 text-xs text-slate-300 flex items-center justify-between">
                <span>Updated Full Name Preview:</span>
                <span className="font-bold text-blue-400 font-mono text-sm">
                  {formData.first_name} {formData.middle_name} {formData.last_name}
                </span>
              </div>
            </div>
          )}

          {/* STEP 2: Birth, Aadhar, Religion & Category */}
          {step === 2 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="border-b border-slate-800 pb-3">
                <h4 className="text-lg font-bold text-white">7 - 12. Identity, Religion & Category</h4>
                <p className="text-xs text-slate-400">Official Indian demographic classification</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    7. Place of Birth *
                  </label>
                  <input
                    type="text"
                    required
                    value={formData.place_of_birth}
                    onChange={(e) => setFormData({ ...formData, place_of_birth: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    8. Date of Birth (DOB) *
                  </label>
                  <input
                    type="date"
                    required
                    value={formData.dob}
                    onChange={(e) => setFormData({ ...formData, dob: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    9. Aadhar Number *
                  </label>
                  <input
                    type="text"
                    required
                    maxLength={14}
                    value={formData.aadhar_no}
                    onChange={(e) => setFormData({ ...formData, aadhar_no: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white font-mono text-sm tracking-wider focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    10. Gender *
                  </label>
                  <select
                    value={formData.gender}
                    onChange={(e) => setFormData({ ...formData, gender: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  >
                    <option value="Male">1) Male</option>
                    <option value="Female">2) Female</option>
                    <option value="Trans Gender">3) Trans Gender</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    11. Minority Religion *
                  </label>
                  <select
                    value={formData.religion}
                    onChange={(e) => setFormData({ ...formData, religion: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  >
                    <option value="Non-Minority">0) Non-Minority</option>
                    <option value="Muslim">1) Muslim</option>
                    <option value="Christian">2) Christian</option>
                    <option value="Buddhist">3) Buddhist</option>
                    <option value="Sikh">4) Sikh</option>
                    <option value="Parsi">5) Parsi</option>
                    <option value="Jain">6) Jain</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    12. Caste Category *
                  </label>
                  <select
                    value={formData.category}
                    onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  >
                    <option value="SC">1) SC</option>
                    <option value="ST">2) ST</option>
                    <option value="VJ(A)">3) VJ(A)</option>
                    <option value="NT(B)">4) NT(B)</option>
                    <option value="NT(C)">5) NT(C)</option>
                    <option value="NT(D)">6) NT(D)</option>
                    <option value="OBC">7) OBC</option>
                    <option value="SBC">8) SBC</option>
                    <option value="OPEN">9) OPEN</option>
                    <option value="MARATHA">10) MARATHA (ESBC)</option>
                    <option value="MUSLIM (SBC-A)">11) MUSLIM (SBC-A)</option>
                  </select>
                </div>
              </div>
            </div>
          )}

          {/* STEP 3: Address & Contact */}
          {step === 3 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="border-b border-slate-800 pb-3">
                <h4 className="text-lg font-bold text-white">5 & 6. Residential Address & Contact</h4>
                <p className="text-xs text-slate-400">Communication details and Pin Code</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    5. Residential Address *
                  </label>
                  <textarea
                    rows={3}
                    required
                    value={formData.address}
                    onChange={(e) => setFormData({ ...formData, address: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    Pin Code (6 Digits) *
                  </label>
                  <input
                    type="text"
                    required
                    maxLength={6}
                    value={formData.pin_code}
                    onChange={(e) => setFormData({ ...formData, pin_code: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white font-mono text-sm tracking-widest focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    6. Primary Mobile No *
                  </label>
                  <input
                    type="tel"
                    required
                    maxLength={10}
                    value={formData.phone}
                    onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white font-mono text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>
              </div>
            </div>
          )}

          {/* STEP 4: Academic Setup */}
          {step === 4 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="border-b border-slate-800 pb-3">
                <h4 className="text-lg font-bold text-white">Academic Division & Section</h4>
                <p className="text-xs text-slate-400">Update division, standard, section & stream</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    Academic Division *
                  </label>
                  <select
                    value={formData.division}
                    onChange={(e) => handleDivisionChange(e.target.value)}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  >
                    <option value="Pre-Primary">Pre-Primary (CBSE Pattern)</option>
                    <option value="School Section">School Section (1st - 10th)</option>
                    <option value="Junior College">Junior College (11th & 12th)</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    Class / Standard *
                  </label>
                  <input
                    type="text"
                    required
                    value={formData.standard}
                    onChange={(e) => setFormData({ ...formData, standard: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase mb-2">
                    Section *
                  </label>
                  <select
                    value={formData.section}
                    onChange={(e) => setFormData({ ...formData, section: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500"
                  >
                    <option value="A">Division A</option>
                    <option value="B">Division B</option>
                    <option value="C">Division C</option>
                  </select>
                </div>
              </div>

              {formData.division === "Junior College" && (
                <div className="p-5 bg-blue-500/10 border border-blue-500/30 rounded-xl space-y-3">
                  <div className="flex items-center gap-2 text-blue-400 font-semibold text-sm">
                    <GraduationCap className="w-5 h-5" />
                    <span>Junior College Stream Selection</span>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    {["Science", "Commerce", "Arts"].map((str) => (
                      <button
                        key={str}
                        type="button"
                        onClick={() => setFormData({ ...formData, stream: str })}
                        className={`p-3 rounded-xl font-bold text-sm border transition-all ${
                          formData.stream === str
                            ? "bg-blue-600 border-blue-400 text-white shadow-lg shadow-blue-500/30"
                            : "bg-slate-900/60 border-slate-700 text-slate-400 hover:text-white"
                        }`}
                      >
                        {str} Stream
                      </button>
                    ))}
                  </div>
                </div>
              )}
            </div>
          )}

          {/* STEP 5: Photo, Review & Document Verification */}
          {step === 5 && (
            <div className="space-y-6 animate-fadeIn">
              <div className="border-b border-slate-800 pb-3">
                <h4 className="text-lg font-bold text-white">Photo, Signature & Proof Verification</h4>
                <p className="text-xs text-slate-400 font-medium">Verify photo, signature & details before saving updates</p>
              </div>

              {/* Photo, Signature & Aadhar Upload Boxes */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {/* Candidate Photo Box */}
                <div className="p-4 bg-slate-950 border border-dashed border-slate-700 rounded-xl text-center flex flex-col items-center justify-center space-y-2">
                  <span className="text-[11px] font-bold text-slate-300 uppercase">CANDIDATE PHOTO *</span>
                  {photoPreview ? (
                    <div
                      onClick={() => setPreviewModal({ isOpen: true, url: photoPreview, title: "Candidate Photo" })}
                      className="group relative cursor-pointer overflow-hidden rounded border border-slate-600 shadow transition-all hover:scale-105"
                      title="Click to view full resolution"
                    >
                      <img src={photoPreview} alt="Candidate Photo" className="w-24 h-28 object-cover" />
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[10px] text-white font-bold transition-opacity">
                        View Resolution
                      </div>
                    </div>
                  ) : (
                    <div className="w-24 h-28 bg-slate-900 border border-slate-800 rounded flex flex-col items-center justify-center text-slate-500 text-[11px]">
                      Passport Size
                    </div>
                  )}
                  <label className="cursor-pointer px-3 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded text-xs font-bold transition-colors">
                    Upload Photo
                    <input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
                  </label>
                </div>

                {/* Candidate Signature Box */}
                <div className="p-4 bg-slate-950 border border-dashed border-slate-700 rounded-xl text-center flex flex-col items-center justify-center space-y-2">
                  <span className="text-[11px] font-bold text-slate-300 uppercase">SIGNATURE *</span>
                  {signaturePreview ? (
                    <div
                      onClick={() => setPreviewModal({ isOpen: true, url: signaturePreview, title: "Candidate Signature" })}
                      className="group relative cursor-pointer overflow-hidden rounded border border-slate-600 bg-white p-1 shadow transition-all hover:scale-105"
                      title="Click to view full resolution"
                    >
                      <img src={signaturePreview} alt="Candidate Signature" className="w-36 h-14 object-contain" />
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[10px] text-white font-bold transition-opacity">
                        View Resolution
                      </div>
                    </div>
                  ) : (
                    <div className="w-36 h-14 bg-slate-900 border border-slate-800 rounded flex flex-col items-center justify-center text-slate-500 text-[11px]">
                      Signature
                    </div>
                  )}
                  <label className="cursor-pointer px-3 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded text-xs font-bold transition-colors">
                    Upload Signature
                    <input type="file" accept="image/*" className="hidden" onChange={handleSignatureUpload} />
                  </label>
                </div>

                {/* Optional Aadhar Card Front Upload Box */}
                <div className="p-4 bg-slate-950 border border-dashed border-slate-700 rounded-xl text-center flex flex-col items-center justify-center space-y-2">
                  <span className="text-[11px] font-bold text-amber-400 uppercase">AADHAR FRONT (OPTIONAL)</span>
                  {aadharFrontPreview ? (
                    <div
                      onClick={() => setPreviewModal({ isOpen: true, url: aadharFrontPreview, title: "Aadhar Card Front Side" })}
                      className="group relative cursor-pointer overflow-hidden rounded border border-slate-600 shadow transition-all hover:scale-105"
                      title="Click to view full resolution"
                    >
                      <img src={aadharFrontPreview} alt="Aadhar Front" className="w-36 h-16 object-cover" />
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[10px] text-white font-bold transition-opacity">
                        View Resolution
                      </div>
                    </div>
                  ) : (
                    <div className="w-36 h-16 bg-slate-900 border border-slate-800 rounded flex flex-col items-center justify-center text-slate-500 text-[11px]">
                      Aadhar Front
                    </div>
                  )}
                  <label className="cursor-pointer px-3 py-1 bg-amber-600 hover:bg-amber-500 text-white rounded text-xs font-bold transition-colors">
                    Upload Front Side
                    <input type="file" accept="image/*,.pdf" className="hidden" onChange={handleAadharFrontUpload} />
                  </label>
                </div>

                {/* Optional Aadhar Card Back Upload Box */}
                <div className="p-4 bg-slate-950 border border-dashed border-slate-700 rounded-xl text-center flex flex-col items-center justify-center space-y-2">
                  <span className="text-[11px] font-bold text-amber-400 uppercase">AADHAR BACK (OPTIONAL)</span>
                  {aadharBackPreview ? (
                    <div
                      onClick={() => setPreviewModal({ isOpen: true, url: aadharBackPreview, title: "Aadhar Card Back Side" })}
                      className="group relative cursor-pointer overflow-hidden rounded border border-slate-600 shadow transition-all hover:scale-105"
                      title="Click to view full resolution"
                    >
                      <img src={aadharBackPreview} alt="Aadhar Back" className="w-36 h-16 object-cover" />
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center text-[10px] text-white font-bold transition-opacity">
                        View Resolution
                      </div>
                    </div>
                  ) : (
                    <div className="w-36 h-16 bg-slate-900 border border-slate-800 rounded flex flex-col items-center justify-center text-slate-500 text-[11px]">
                      Aadhar Back
                    </div>
                  )}
                  <label className="cursor-pointer px-3 py-1 bg-amber-600 hover:bg-amber-500 text-white rounded text-xs font-bold transition-colors">
                    Upload Back Side
                    <input type="file" accept="image/*,.pdf" className="hidden" onChange={handleAadharBackUpload} />
                  </label>
                </div>
              </div>

              {/* Mandatory Document Verification Card if Name or DOB changed */}
              {requiresProofDocument && (
                <div className="p-5 bg-amber-500/10 border border-amber-500/40 rounded-xl space-y-3">
                  <div className="flex items-center gap-2 text-amber-400 font-bold text-sm">
                    <AlertTriangle className="w-5 h-5" />
                    <span>Proof Verification Required (Name or Date of Birth Changed)</span>
                  </div>
                  <p className="text-xs text-slate-300">
                    You have modified candidate core details ({isNameChanged ? "Full Name" : ""}{" "}
                    {isNameChanged && isDobChanged ? "&" : ""} {isDobChanged ? "Date of Birth" : ""}).
                    Official policy requires uploading a valid verification document (Birth Certificate / Aadhar Card).
                  </p>

                  <div className="flex items-center gap-4 pt-1">
                    <label className="cursor-pointer px-4 py-2.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-bold shadow-md transition-all flex items-center gap-2">
                      <Upload className="w-4 h-4" />
                      <span>{proofFile ? proofFile.name : "Attach Birth Cert / Aadhar Card"}</span>
                      <input
                        type="file"
                        accept=".pdf,image/*"
                        className="hidden"
                        onChange={(e) => {
                          if (e.target.files && e.target.files[0]) {
                            setProofFile(e.target.files[0]);
                            setErrorMsg(null);
                          }
                        }}
                      />
                    </label>
                    {proofFile ? (
                      <span className="text-xs text-emerald-400 font-bold flex items-center gap-1">
                        <CheckCircle2 className="w-4 h-4" /> Document Verified & Attached
                      </span>
                    ) : (
                      <span className="text-xs text-rose-400 font-semibold">
                        * Upload required to save changes
                      </span>
                    )}
                  </div>
                </div>
              )}

              {/* Summary Review Card */}
              <div className="p-5 bg-slate-950/90 rounded-xl border border-slate-800 space-y-3">
                <h5 className="text-xs font-bold uppercase tracking-wider text-blue-400">
                  Updated Record Summary
                </h5>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                  <div>
                    <span className="text-slate-500 block">Candidate Name:</span>
                    <span className="font-bold text-white">
                      {formData.first_name} {formData.middle_name} {formData.last_name}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Mother's Name:</span>
                    <span className="font-bold text-white">{formData.mother_name}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Date of Birth:</span>
                    <span className="font-bold text-white">{formData.dob}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Aadhar Number:</span>
                    <span className="font-mono font-bold text-white">{formData.aadhar_no}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Category / Religion:</span>
                    <span className="font-bold text-amber-400">
                      {formData.category} ({formData.religion})
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Division & Standard:</span>
                    <span className="font-bold text-white">
                      {formData.division} - Std {formData.standard} ({formData.section})
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Mobile Phone:</span>
                    <span className="font-bold text-white">{formData.phone}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 block">GR Number:</span>
                    <span className="font-mono font-bold text-blue-400">{student.gr_no}</span>
                  </div>
                </div>
              </div>
            </div>
          )}

          {/* Modal Footer Wizard Controls */}
          <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
            {step > 1 ? (
              <button
                type="button"
                onClick={() => setStep(step - 1)}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-slate-700 bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 text-xs font-semibold transition-all"
              >
                <ArrowLeft className="w-4 h-4" />
                Previous Step
              </button>
            ) : (
              <div />
            )}

            {step < 5 ? (
              <button
                type="button"
                onClick={handleNextStep}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-lg shadow-blue-600/20 transition-all"
              >
                <span>Next Step</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            ) : (
              <button
                type="button"
                onClick={handleFinalSave}
                disabled={saving || (requiresProofDocument && !proofFile)}
                className="flex items-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-bold text-xs shadow-xl shadow-emerald-600/30 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
                <Save className="w-4 h-4" />
                <span>{saving ? "Saving Changes..." : "Save Record Changes"}</span>
              </button>
            )}
          </div>
        </form>
      </div>

      <ImagePreviewModal
        isOpen={previewModal.isOpen}
        imageUrl={previewModal.url}
        title={previewModal.title}
        onClose={() => setPreviewModal({ isOpen: false, url: null, title: "" })}
      />
    </div>
  );
}
